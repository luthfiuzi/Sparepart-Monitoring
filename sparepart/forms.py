...
from sparepart.models import SparePart, HistoryBarangMasuk, HistoryBarangKeluar
...
class BarangKeluarForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('tanggal', 'sparepart', 'karyawan', 'jumlah')
        widgets = {
            'tanggal': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}), # 'placeholder': 'Keluarkan nama sparepart
            'sparepart': forms.Select(attrs={'class': 'form-control'}),
            'karyawan': forms.Select(attrs={'class': 'form-control'}),
            'jumlah': forms.NumberInput(attrs={'class': 'form-control'}), # 'placeholder': 'Keluarkan jumlah sparepart
        }
    # add inline form using formhelper
    def __init__(self, *args, **kwargs):
        super(BarangKeluarForm, self).__init__(*args, **kwargs)
        self.helper = FormHelper()
        self.helper.form_tag = False
        self.helper.layout = Layout(
            layout.Row(
